...
- New repo under **`Shiti-Sharma09`** (need: exact name — default
  `referral_card` — and private or public). Feature-branch → PR workflow.
...
- `POST /leads` (auth + event access): body = fields + `event_id` +
  ...
  Row = `[ISO timestamp, user email, event name, name, company, title, email,
  phone, notes, assigned_to]`.
...
- Repo **`referral_card`**, **public**, created under **`Shiti-Sharma09`**.
...
| # | Decision |
|---|---|
| Repo | `referral_card`, **public**, under `Shiti-Sharma09`; branch + PR per phase |
